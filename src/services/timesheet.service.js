...
const getTimesheetById = async (id) => {
  ...
  if (!timesheet) {
    throw new Error('Get a timesheet not found');
  }
  return timesheet;
};

getTimeSheetByDateTime = async (time) => {
  const timeSheet = await Timesheet.find({
    datetime: {
      $gte: time.StartDayTime,
      $lte: time.EndDayTime,
    },
  });
  if (!timeSheet) {
    throw new Error('not found timesheetBy dateTime');
  }
  return timeSheet;
};
...
const updateTimesheetById = async (timesheetId, updateBody) => {
  ...
  if (!timesheet) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Timesheet update not found');
  }
