...
const getTimesheet = catchAsync(async (req, res) => {
  ...
  if (!timesheet) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Timesheet not found api');
  }
  res.send(timesheet);
});

const getTimeSheetByDateTime = catchAsync(async (req, res) => {
  const filter = pick(req.query, ['StartDayTime', 'EndDayTime']);
  const timesheet = await timesheetService.getTimeSheetByDateTime(filter);
  if (!timesheet) {
    throw new Error(httpStatus.NOT_FOUND, 'controller not found timesheet');
  }
  res.send(timesheet);
});
